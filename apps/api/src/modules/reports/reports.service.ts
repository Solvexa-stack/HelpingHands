...
export class ReportsService {
  ...
  async generateProjectSummaryPdf(projectId: number): Promise<Buffer> {
  ...
  async generateFinancialPdf(projectId: number): Promise<Buffer> {
    const project = await this.getProject(projectId);
    const name = project.block.translations[0]?.name || `Project #${projectId}`;
    const collected = project.donations.reduce((s, d) => s + Number(d.amount), 0);
      ...
      if (budgets.length) {
        ...
        budgets.forEach((b) => {
          ...
          doc.fontSize(11).fillColor('#333').text(
            `${bName}: Est. $${Number(b.estimatedAmount).toLocaleString()} | Approved: $${Number(b.approvedAmount ?? 0).toLocaleString()} | Spent: $${Number(b.actualAmount).toLocaleString()}`,
          );
        });
        doc.moveDown(1);
      }
  ...
  async generateProgressPdf(projectId: number): Promise<Buffer> {
    const project = await this.getProject(projectId);
    const name = project.block.translations[0]?.name || `Project #${projectId}`;
    ...
    return new Promise((resolve) => {
      ...
      const printSection = (title: string, items: any[], getLabel: (i: any) => string) => {
  ...
  // ─── Excel: Donations Report ───────────────────────────────────────────────

  async generateDonationsExcel(projectId: number): Promise<Buffer> {
    const project = await this.getProject(projectId);
    ...
    donations.forEach((d) => sheet.addRow({
      id: d.id,
      participant: `${(d.participant as any).firstName} ${(d.participant as any).lastName}`,
      email: (d.participant as any).user?.email || '',
