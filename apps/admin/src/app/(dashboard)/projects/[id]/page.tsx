...
export default function ProjectDetailPage({ params }: { params: { id: string } }) {
  ...
  if (!project) return <div className="py-20 text-center text-gray-400">{t('projects.detail.notFound')}</div>;

  const name = getTranslation(project.block?.translations || [])?.name || t('projects.unnamedFallback');
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <Link href={`/projects/${id}/edit`} className="btn-primary btn-md gap-2">
          <Pencil className="w-4 h-4" /> {t('projects.detail.editProject')}
        </Link>
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="card p-6 space-y-4">
            <h2 className="font-semibold text-gray-900">{t('projects.detail.fundingProgress')}</h2>
            <div className="space-y-2">
              <div className="flex justify-between text-sm">
                <span className="text-gray-500">{t('projects.detail.collected')}</span>
              ...
              <p className="text-end text-sm font-semibold text-gray-700">{t('projects.detail.fundedPercent', { percent: progression.toFixed(1) })}</p>
            </div>

            {/* Mini stats */}
            <div className="grid grid-cols-3 gap-4 pt-2">
              {[
                { icon: CheckCircle, label: t('dashboard.approved'), value: statusCount['approved'] || 0, color: 'text-green-600 bg-green-50' },
                { icon: Clock, label: t('dashboard.pending'), value: statusCount['pending'] || 0, color: 'text-yellow-600 bg-yellow-50' },
                { icon: XCircle, label: t('projects.detail.rejected'), value: statusCount['rejected'] || 0, color: 'text-red-600 bg-red-50' },
          ...
          <div className="card overflow-hidden">
            <div className="px-5 py-4 border-b border-gray-100">
              <h2 className="font-semibold text-gray-900">{t('projects.detail.donationsHeading', { count: donations.length })}</h2>
            </div>
            {donations.length === 0 ? (
              <p className="text-center py-10 text-gray-400 text-sm">{t('projects.detail.noDonationsYet')}</p>
            ) : (
        ...
        <div className="space-y-5">
          ...
          <div className="card p-5">
            <span className={cn('badge text-sm px-3 py-1', project.isCompleted ? 'bg-green-100 text-green-700' : 'bg-blue-100 text-blue-700')}>
              {project.isCompleted ? t('projects.completed') : t('common.active')}
            </span>
          </div>

          {/* Details */}
          <div className="card p-5 space-y-4">
            <h3 className="font-semibold text-gray-900">{t('projects.detail.details')}</h3>
            {[
              { icon: TrendingUp, label: t('projects.category'), value: project.categoryNode?.name ?? project.category ?? '—' },
              { icon: DollarSign, label: t('projects.target'), value: formatCurrency(target, undefined, locale) },
              { icon: MapPin, label: t('projects.detail.location'), value: project.location || '—' },
              { icon: Calendar, label: t('projects.detail.startDate'), value: project.expectedStartDate ? formatDate(project.expectedStartDate, locale) : '—' },
              { icon: Calendar, label: t('projects.detail.completionLabel'), value: project.dateOfCompletion ? formatDate(project.dateOfCompletion, locale) : '—' },
              { icon: Users, label: t('participants.colDonations'), value: String(project._count?.donations ?? 0) },
          ...
          <div className="card p-5 space-y-3">
            <h3 className="font-semibold text-gray-900">{t('projects.detail.projectManagement')}</h3>
            {[
              { href: `/projects/${id}/execution`, icon: Settings2, label: t('projects.detail.execution'), desc: t('projects.detail.executionDesc'), color: 'bg-blue-100 text-blue-600' },
              { href: `/projects/${id}/financial`, icon: Wallet, label: t('projects.detail.financialLabel'), desc: t('projects.detail.financialDesc'), color: 'bg-green-100 text-green-600' },
              { href: `/projects/${id}/milestones`, icon: Flag, label: t('projects.detail.milestonesLabel'), desc: t('projects.detail.milestonesDesc'), color: 'bg-purple-100 text-purple-600' },
          ...
          {project.financialOfficer && (
            <div className="card p-5 space-y-3">
              <h3 className="font-semibold text-gray-900">{t('projects.detail.financialOfficer')}</h3>
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-400">{t('projects.detail.financialOfficer')}</p>
                </div>
              </div>
            </div>
          )}
