...
export default function BlogsPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['blocks-blog', search, page],
    queryFn: () => blocksApi.list({ category: 'blog', search: search || undefined, activeOnly: false, page, limit: LIMIT }),
  });
                  ...
                  <tr key={b.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="table-cell">
                      <div className="flex gap-1.5">
                        <Link href={`/content/blogs/${b.id}/edit`} className="btn-ghost btn-sm p-1.5 rounded-lg text-gray-500">
                          <Pencil className="w-3.5 h-3.5" />
                        </Link>
                        <button onClick={() => toggleMutation.mutate(b.id)} className="btn-ghost btn-sm p-1.5 rounded-lg text-gray-500">
                          {b.isActive ? <ToggleRight className="w-3.5 h-3.5 text-green-500" /> : <ToggleLeft className="w-3.5 h-3.5" />}
                        </button>
                        <button onClick={() => setDeleteId(b.id)} className="btn btn-sm p-1.5 bg-red-50 text-red-500 hover:bg-red-100 rounded-lg">
